<script lang="ts">
  ...
  const onSubmit = async ({skipNsecWarning = false} = {}) => {
    ...
    if ($userSettings.send_delay > 0) {
      await showToast({
        ...
        onCancel: () => {
          router.at("services/create").open()
        },
      })
    }
  ...
  document.title = "Create Service Listing"
</script>

<form on:submit|preventDefault={() => onSubmit()}>
  <Content size="lg">
    <FlexColumn class="gap-4">
      <Heading>Create Service Listing</Heading>

      <Field label="Service Title *">
        <Input type="text" bind:value={title} placeholder="e.g., Web Development Services" />
      </Field>
      ...
      <Field label="Category">
        <Input type="text" bind:value={category} placeholder="e.g., Technology, Consulting, Design" />
      </Field>

      <Field label="Service Type">
        <Input type="text" bind:value={serviceType} placeholder="e.g., Remote, On-site, Hybrid" />
      </Field>

      <Field label="Location">
        <Input type="text" bind:value={location} placeholder="City, Country or Remote" />
      </Field>

      <Field label="Price (in sats)">
        <Input type="text" bind:value={price} placeholder="e.g., 100000" />
      </Field>

      <Field label="Business Hours">
        <Input type="text" bind:value={businessHours} placeholder="e.g., Mon-Fri 9am-5pm EST" />
      </Field>
